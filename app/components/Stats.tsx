  ...
  return (
    <div className="py-16 md:py-24 px-4 md:px-24">
      <div className="max-w-8xl mx-auto text-start">
        <h2 className="text-4xl md:text-5xl font-semibold text-[#22D2EE] mb-4">Trusted By Millions</h2>
        <p className="text-gray-300 text-lg md:text-xl mb-8 md:mb-12">Join the Largest Challan Resolution Platform in India</p>
        <div className="mt-12 md:mt-16 grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-8">
          {stats.map((stat) => (
            <div key={stat.label} className="flex flex-col gap-3 items-start text-center p-4 md:py-6 md:pl-6 border border-gray-700">
              <div className="h-25 w-25 flex items-center justify-start">
                {stat.icon}
              </div>
              <p className="text-2xl md:text-4xl font-bold mt-2 text-white">{stat.value}</p>
              <p className="text- text-base md:text-xl mt-1 pb-6">{stat.label}</p>
            </div>
          ))}
        </div>
      </div>
