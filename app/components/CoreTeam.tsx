        ...
        <div className="flex flex-col md:flex-row items-center text-center md:text-left gap-6 md:gap-8 mb-6">
          ...
          <div>
            <div className="flex flex-col">
              ...
              <span className="text-3xl md:text-4xl lg:text-5xl font-bold tracking-wider text-[#00A2BB] dark:text-[#22D3EE]">
                {lastName.toUpperCase()}
              </span>
            </div>
            <p className="text-base md:text-lg text-gray-700 dark:text-gray-300 mt-2">{member.title}</p>
            <a href={member.linkedin} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 mt-4 border border-gray-400 dark:border-gray-600 px-4 py-2 rounded-lg text-xs md:text-sm text-gray-700 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-gray-700">
              <Linkedin size={16}/> LinkedIn
            </a>
          </div>
        </div>
        <div className="text-gray-600 dark:text-gray-400 space-y-4 text-xs md:text-sm leading-relaxed max-h-[50vh] md:max-h-none overflow-y-auto">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-x-8 gap-y-12">
          {team.map(member => (
            <div key={member.name} className="text-center cursor-pointer" onClick={() => setSelectedMember(member)}>
              ...
              <h3 className="text-base md:text-lg font-bold text-black dark:text-white mt-4">{member.name}</h3>
